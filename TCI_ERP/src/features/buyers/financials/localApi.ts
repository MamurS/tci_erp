...
export function useCreateLocalStatement(buyerId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['buyers', buyerId, 'statements'] })
      void queryClient.invalidateQueries({ queryKey: ['buyers'] })
    },
  })
}

export function useUpdateLocalStatement(buyerId: string, statementId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async (input: LocalStatementSaveInput) => {
