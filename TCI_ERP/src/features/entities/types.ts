/** Row types mirroring the `tci` schema (migrations 0005, 0015). */

export type StatementKind = 'annual' | 'quarterly'
export type StatementUnit = 'units' | 'thousands' | 'millions'

/** tci.legal_entities — the unified registry (Phase 3a). */
...
export interface EntityWithRefs extends LegalEntity {
...
/** tci.v_entity_roles row — roles are computed, never assigned. */
...
/** tci.similar_entities() row (pg_trgm fuzzy dedup suggestion). */
